...
void loop() {
  ...
  if (SerialBT.available()) {
    ...
    } else if (strncmp(payload.c_str(), "SSID_NAME=", 10) == 0) {
      SSID_NAME = payload.substring(10);
      
    } else if (strncmp(payload.c_str(), "SSID_PASS=", 10) == 0) {
      SSID_PASS = payload.substring(10);
      
    } else if (strncmp(payload.c_str(), "API_URL=", 8) == 0) {
      API_URL = payload.substring(8);
  ...
  if(connecting) {
    delay(1000);
    //connect_network(SSID_NAME.c_str(), SSID_PASS.c_str());
    String b = "";
    String c = "";
    
    connect_network(b.c_str(), c.c_str());
    connecting = false;
  }
  ...
  ApiResponse response = api_get("https://robot.icmsrv.com/motion.txt");
